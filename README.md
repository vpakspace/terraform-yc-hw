...
- 🧹 **Все созданные ресурсы удалены** одной командой — `terraform destroy` →
  `Destroy complete! Resources: 7 destroyed` (обе ВМ, обе подсети, сеть, NAT gateway, route table).
  В облаке платных ресурсов не осталось (проверено `yc compute/vpc ... list` — пусто).
